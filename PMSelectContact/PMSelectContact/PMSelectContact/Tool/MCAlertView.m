...
@interface MCAlertView () <UITextViewDelegate>
...
+ (instancetype)popAlertViewWithTitle:(NSString *)title
    ...
    MCAlertView *view = [[MCAlertView alloc] initWithTitle:title
                                                   message:message
...
//    MCAlertView *view = [[MCAlertView alloc] initWithInputTitle:title inputText:inputText leftButtonTitle:leftButtonTitle rightButtonTitle:rightButtonTitle];
//    return view;
//}
...
@implementation MCAlertView (MCloudAddition)
...
+ (instancetype)popAlertViewWithAppletDict:(NSDictionary *)dict sameAccout:(BOOL)sameAccout {
    if (sameAccout) {
        ...
        MCAlertView *view = [MCAlertView popAlertViewWithTitle:@"温馨提示" message:@"当前账号与小程序端账号不一致，无法查看来自小程序的文件" leftButtonTitle:@"取消" rightButtonTitle:@"切换账号"];
         return view;
    }
}
...
+ (BOOL)isThereAlertWithIdentifiers:(NSSet *)identifiers {
    ...
    for (UIView *element in views) {
        if ([element isKindOfClass:[MCAlertView class]]) {
            MCAlertView *existentAlertView = (MCAlertView *)element;
...
@implementation MCAlertView (OutLink)
...
@end
@implementation MCAlertView (MarketPassword)
